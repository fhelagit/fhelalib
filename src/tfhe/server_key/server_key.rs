...
impl<S: TFHESchema, P_lwe: LWE_CT_Params<S>, P_glwe: LWE_CT_Params<S>> BootstrappingKey<S, P_lwe, P_glwe> {
  ...
  pub struct KeyswitchingKey<S: TFHESchema, P_lwe_old: LWE_CT_Params<S>, P_lwe: LWE_CT_Params<S>>{pub key: P_lwe::ContainerType, phantom1: PhantomData<P_lwe>, phantom2: PhantomData<P_lwe_old>}

  impl<S: TFHESchema, P_lwe_old: LWE_CT_Params<S>, P_lwe: LWE_CT_Params<S>> KeyswitchingKey<S, P_lwe_old, P_lwe> {
      pub fn from_polynomial_list(data: P_lwe::ContainerType) -> Self {
          KeyswitchingKey{key: data, phantom1: PhantomData, phantom2: PhantomData}
      }
