...
fn bench_bootstrap(c: &mut Criterion) {
    ...
    let encrypted_message: GLWECiphertext<MySchema, LWE_Params<MySchema>> =
        sk_old.encrypt(&message);

    let bsk: BootstrappingKey<MySchema, LWE_Params<MySchema>, GLWE_Params<MySchema>> =
        sk_new.create_bootstrapping_key(&sk_old);
    let extracted_key = sk_new.extract_key::<LWE_Params_after_extraction<MySchema>>();
    let ksk =
        sk_old.create_keyswitching_key::<LWE_Params_after_extraction<MySchema>>(&extracted_key);
    ...
    c.bench_function("eval", |b| {
        b.iter(|| {
            let _: GLWECiphertext<MySchema, LWE_Params<MySchema>> =
                eval_key.eval(&encrypted_message, &f);
        })
    });
}
